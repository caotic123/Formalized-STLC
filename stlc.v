...
Inductive Enviroment := 
  |empty : Enviroment
  |bind : forall T {H : T <> Udnf}, Expression -> Enviroment -> Enviroment.
...
Definition ArrowIsDef : forall T1 T2, Arrow T1 T2 <> Udnf. by []. Defined.

Inductive typed_rules : T* -> Symbol Value -> Expression -> Type :=
  |Tvar : forall (env : Enviroment) (s : Symbol _) T {H' : T <> Udnf}, (env |- {(Var s T) ::: T}) -> typed_rules env s (Var s T)
  |TAbs : forall (env : Enviroment) (k : Symbol Value) x T2 {H' : T2 <> Udnf} s,
  ...
  |TApp : forall (env : Enviroment) (k : Symbol Value) (x : Expression) T T' T1 T2 {H : T2 <> Udnf},
    typed_rules env k x -> (env |- {T ::: Arrow T1 T2}) -> (env |- {T' ::: T1}) -> 
      (fun t => (typed_rules (@bind T2 H t env) k t)) (App T T' T2).

Theorem well_typed_context_has_type : forall {K} (env : Enviroment) (s : Symbol _) (H : typed_rules env s K), (env |- K) -> K ∈ env.
intros.
destruct H0;destruct (varType K env); cbv.
discriminate.
discriminate.
have : forall env, has_type Udnf env = false.
...
auto.
simpl; trivial.
assert (forall x, x = false -> x <> true).
move => H'.
destruct H'; move => SH.
discriminate.
discriminate.
...
Lemma H0_symbol : forall {T} (s : Symbol T), same_symbol s s = true. 
move => f; elim; trivial.
Defined.

Hint Resolve H0_symbol.

Lemma H1_expression : forall e, same_expression e e = true.

have : forall s', same_type s' s' = true.
elim.
move => //=.
move => Eq k k' H.
move => /=.
...
Lemma H3_type : forall e, same_type e e = true.
elim.
move => q /=; by [].
move => q k' v H' /=.
rewrite k' //.
...
Theorem type_context_abs : 
  forall (env : Enviroment) (s : Symbol _) T k y (H : typed_rules env s (Abs k y T)), (env |- {(Abs k y T) ::: T}).
intros.
...
Theorem Types_of_terms_are_well_defined : forall T x s env, typed_rules env s x -> varType x env = T -> T <> Udnf.
intros.
destruct H.
...
Lemma conservation_of_types : forall T x s env, typed_rules env s x -> varType x env = T -> type_of_T x = T.
intros.
...
Theorem type_context : 
  forall (env : Enviroment) (s : Symbol _) T K (H : typed_rules env s K), type_of_T K = T -> (env |- {K ::: T}).
intros.
...
Lemma eq_symb : forall {T} (x : Symbol T) y, same_symbol x y = true -> x = y.
intros.
...
Lemma eq_type : forall k y, same_type k y = true-> k = y.
...
Lemma False_injection_nextC : forall (t : TypeDef) t0, t = Arrow t0 t -> False.
induction t.
intros.
discriminate.
move => H H1.
...
Theorem lambda_type_return : 
  forall (env : Enviroment) (s : Symbol _) k y T1 T2 x (H : typed_rules env s x), x = (Abs k y (Arrow T1 T2)) ->
...
Lemma strong_enviroment_conservation : forall T2 env y, same_type (varType y env) T2 = true -> varType y env = T2.
move => t2 d; elim.
move => y g Heq.
induction y.
destruct d.
...
Theorem lambda_symbol_uniqueness : 
  forall (env : Enviroment) (s : Symbol _) k y T
    (H : typed_rules env (next _ s) (Abs k y T)),
    k = Var s (type_of_T k).
intros.
remember s.
remember (Abs k y T).
...
Theorem well__lambda_typed_context_has_type : 
  forall (env : Enviroment) (s : Symbol _) k y T (H : typed_rules env (next _ s) (Abs k y T)) T1 T2 R1 R2,
    (env |- {k ::: T1}) -> (env |- {(Abs k y T) ::: Arrow R1 R2}) -> R2 = T2.
